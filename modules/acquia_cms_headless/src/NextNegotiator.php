...
class NextNegotiator {
   ...
   *   The request.
   *
   * @return \Drupal\acquia_cms_headless\Entity\NextConsumer|null
   *   The consumer.
   *
   * @throws \Drupal\consumers\MissingConsumer
   */
  protected function doNextNegotiateFromRequest(Request $request) {
    ...
    try {
      /** @var \Drupal\acquia_cms_headless\Entity\NextConsumer $consumer */
   ...
   *   The consumer.
   *
   * @throws \Drupal\acquia_cms_headless\MissingNextConsumer
   */
  public function negotiateFromRequest(Request $request = NULL) {
    ...
    $consumer = $this->doNextNegotiateFromRequest($request);
   ...
   *
   * @return \Drupal\acquia_cms_headless\Entity\NextConsumer
   *   The consumer entity.
   *
   * @throws \Drupal\acquia_cms_headless\MissingNextConsumer
   */
  protected function loadDefaultConsumer() {
    ...
    if (!$consumer_id) {
      ...
      throw new MissingNextConsumer('Unable to find the default consumer.');
    }
    /** @var \Drupal\acquia_cms_headless\Entity\NextConsumer $consumer */
  ...
  public function setEntityStorage(EntityTypeManagerInterface $entity_type_manager) {
    $this->storage = $entity_type_manager->getStorage('nextconsumer');
  }
